...
RESULTS_DIR = PROJECT_ROOT / "results" # <-- 确保指向正确的 results 目录！
...
def main():
    ...
    # --- 让优化参数变成可选，并且去掉默认值！ ---
    parser.add_argument("--opt_steps", type=int, help="覆盖 inverse_opt 中的优化步数 (可选)")
    parser.add_argument("--opt_lr", type=float, help="覆盖 inverse_opt 中的优化学习率 (可选)")
    parser.add_argument("--opt_lbfgs", type=int, help="覆盖 inverse_opt 中的 LBFGS 精修步数 (可选)")
